...
## Execution boundaries (MSEP)

`mira_agent.msep` is the client side of the Mira Stateless Execution Protocol,
and the same code a Mira boundary runs: bounded, signed authority travels with
the action, and the receiving boundary verifies it locally instead of asking a
central service on every hop. The port is byte-identical to the control
plane's implementation — `vectors/msep.json` was generated by the server and
the SDK reproduces every digest, signing input and signature in it.

What an agent host can do with it:

```python
from mira_agent.msep import (ExecutionBoundary, KeyCache, TrustEpoch,
                             Envelope, ExecutionState, Permissions)

keys = KeyCache(); keys.add("<key id>", public_bytes)      # distributed by the control plane
boundary = ExecutionBoundary(name="my-tool", key=my_key, keys=keys,
                             epoch=TrustEpoch(current=3), policy_digest=bundle_digest)

inbound = Envelope.from_wire(request.headers["x-msep-envelope"])   # however it arrived
result = boundary.handle(inbound=inbound, state=ExecutionState(...),
                         next_destination="downstream-tool",
                         next_permissions=narrower)
if result.executed:
    forward(result.successor.to_wire())      # fresh, narrower authority for the next hop
boundary.queue.drain()                       # receipts, shipped when convenient
```

Three checks, no I/O: the signature chain and the key; the requested action
against the permission vector the envelope carries; a drift score computed
elsewhere. Authority narrows and never widens across a hop; a successor is
minted by the boundary, never forwarded by the actor; expiry, depth and the
predecessor link are enforced on the object actually produced. The six
dispositions are `release`, `redact`, `gate`, `elevate`, `interdict` and
`recover`; only the first two execute.

Also here: boundary certificates chained to a trust root with attestation
binding and rotation (`trust_root`), freshness bounds stated in numbers
(`freshness`), Red Cards as RFC 8417 Security Event Tokens with a CAEP
companion (`signals`), schema-level deterministic redaction (`redact`),
customer-only key custody for sealed state (`seal`), receipt pseudonymisation
(`residency`) and lineage reconstruction with explicit gaps (`lineage`).

### The client calls

| | |
|---|---|
| `mira.residency()` | where the ledger runs, who holds its key, that zero Liora calls seal a record |
| `mira.boundary_status()` | certificates, freshness, hot path and its measured cost, custody, Red Cards in force |
| `mira.run_chain(txn_id)` | the hop chain for a run: every envelope, verdict and receipt |
| `mira.reinstate(subject)` | lift a terminal Red Card (owner or admin); a new signed event, not an erasure |

### What it costs, and what it cannot claim

With the Rust core built, an Ed25519 verification of an envelope — canonicalise,
encode, verify — costs about 47 µs on a 2-core commodity box; the keyed-MAC
mode for hops inside one trust domain about 12 µs; the deterministic
permission bound under a microsecond. Without the Rust core the same
verification is about 144 µs in pure Python. The two produce identical bytes;
`fast.AVAILABLE` says which is loaded.

Be precise about the latency claim. On the same host a loopback policy call
answers in tens of microseconds, so centralisation is *faster* there; local
verification wins once the round trip to the control plane exceeds roughly
120 µs, which is any real network hop. And an LLM step costs hundreds of
milliseconds, against which either is noise. The reasons that hold at any
